...
def render_generated_mail_display(company_name, selected_coordinator):
    if st.session_state.get('mail_generated', False) and st.session_state.get('generated_content', ''):
        ...
        edited_content = st.text_area(
            ...
            height=400,
            key="generated_email_content"
        )

        # Subheader spanning full width
        st.subheader("📋 Copy to Clipboard:", divider=False)
        
        # Copy button section with columns
        st.markdown("<div style='margin-top: -10px; margin-bottom: 15px;'>", unsafe_allow_html=True)
        
        copy_col1, copy_col2 = st.columns([1, 4])
        
        with copy_col1:
            was_copied = copy_button(
                edited_content,
                key="copy_invite_btn",
        ...
        if was_copied:
            st.toast("Text copied to clipboard!", icon="✅", duration=20000)
